...
const styles = {
  ...
  empathy: {
    ...
    color: T.muted,
    /**
     * 아래 여백 30 → 56 (2026-08-20). **이 화면 위계의 실제 장치다.**
     *
     * [색으로 두 번 시도했고 두 번 부족했다]
     *   공감 문구(읽는 글)와 비선택 탭(누르는 것)이 같은 T.muted였다.
     *   탭을 85%로 내렸더니 대비는 6.28 → 4.91로 움직였는데 **눈으로는
     *   구분되지 않았다** — 지각적으로 15% 차이일 뿐이다.
     *   더 내리는 길도 막혀 있다: 80%=4.53:1, 75%=4.13:1로 AA(4.5)를 깬다.
     *   탭 라벨은 아이콘이 아니라 읽는 글자라 그 선을 넘을 수 없다.
     *
     * [진짜 원인은 근접이었다]
     *   구절 34px ↔ 공감 문구 ↔ 탭 30px.
     *   공감 문구가 **구절보다 탭에 더 가까웠다.** 그러니 탭 줄과 한
     *   덩어리로 묶여 읽혔다 — 색이 무엇이든 근접이 먼저 이긴다.
     *   이제 24 / 56으로 2.3배 벌려 구절 쪽에 붙인다.
     *
     * ⚠ 색을 더 만지기 전에 이 여백부터 볼 것. 이 화면은 T.mist와 T.muted가
     *   각각 서너 역할을 겸하고 있어 **색이 이미 포화 상태**다
     *   (구절·선택 탭·영상 제목이 전부 T.mist, 공감 문구·주제분 헤더·채널명이
     *   전부 T.muted). 층을 더 만들 여지가 색에는 없다. HANDOFF 2.36.
     */
    margin: "0 0 56px",
    wordBreak: "keep-all",
  },
