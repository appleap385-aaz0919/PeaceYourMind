 ...
 *
 *   ⚠ 2026-08-20에 이 표를 배포본에서 다시 재 **값이 그대로임을 확인했다**
 *     (358px · 여유 282px). 같은 날 아래 wrap 여백을 24px로 줄였다가
 *     348px가 됐지만 그 변경은 되돌렸으므로 표는 원래대로 유효하다.
 *
...
const styles = {
   ...
   * 아래 여백 34px — 원래 값이다.
   *
   * 2026-08-20에 24px로 줄였다가 **되돌렸다.** 공감 문구를 구절 쪽으로 당겨
   * 탭 줄에서 떼어내려는 시도였는데, 렌더로 보니 구절 블록이 무거워지고
   * 벌어진 자리는 위계가 아니라 그냥 빈 공간으로 읽혔다.
   * 위계를 만든 것은 여백이 아니라 **탭 줄 위의 구분선**이었다
   * (ResultTabs.jsx). HANDOFF 2.36.
   */
