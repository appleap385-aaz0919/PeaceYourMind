 ...
 *   08-20 여백 24px   18px    211px       348px         들어옴  여유 292px
 *
 *   마지막 행은 아래 wrap의 여백을 34 → 24로 줄인 결과다(위계 개정, HANDOFF 2.36).
 *   글자 크기는 그대로이고 공감 문구가 10px 위로 올라왔을 뿐이라 이 요건은
 *   오히려 여유로워졌다. 두 값 모두 실측이며, 358/282는 배포본에서 다시 재
 *   위 표가 맞다는 것을 확인한 뒤 아랫줄을 더했다.
 *
...
const styles = {
  /**
   * 아래 여백 34 → 24 (2026-08-20).
   *
   * 공감 문구를 구절 쪽으로 당긴다. 전에는 구절과 34px, 탭 줄과 30px이라
   * **공감 문구가 구절보다 탭에 더 가까웠고**, 그래서 읽는 글이 탭 줄의
   * 일부처럼 읽혔다. 색을 두 번 손봤지만(T.muted → 85%) 해결되지 않았다 —
   * 근접(proximity)이 색보다 강한 신호였기 때문이다. HANDOFF 2.36.
   *
   * ⚠ 위 SIZE_STEPS 실측표의 "공감 문구 하단"이 이 값에 걸려 있다.
   *   여백을 10px 줄였으므로 그만큼 위로 올라온다 — 360×640 한 화면
   *   요건은 더 여유로워진다(줄이는 방향이라 안전하다).
   */
  wrap: { margin: "10px 0 24px" },
