...
# 한 **탭**이 폴백으로 채울 수 있는 최대치 (2026-08-26 · 사용자 결정으로 개정).
#
# ⚠⚠ **정책이 뒤집힌 자리다. 되돌리기 전에 읽을 것.**
#   전   12 — "폴백이 화면을 다 덮지 못하게 막는다"(20슬롯의 60%)
#   후   20 — 탭 상한과 같다. **한 탭이 100% 폴백이 될 수 있다.**
#
#   근거(사용자 2026-08-26): 폴백 층은 헤더가 "딱 맞는 건 아니지만"이라고 이미
#   밝히고 있으므로 정직성이 훼손되지 않고, **선택지가 많은 편이 사용자에게 낫다.**
#   탭별 상한으로 바뀌며 주제분이 0인 탭이 실제로 생기는데(anger.irritation의
#   [말씀]), 거기서 12로 끊으면 이유 없이 8자리를 비우는 셈이다.
#
# ⛔ "폴백이 과반이다"를 결함으로 보고 이 값을 다시 낮추지 말 것. 그것은
#   **공급 부족의 신호**이지 이 상수의 문제가 아니다 — theme_fallback_heavy
#   경보가 탭별로 그 사실을 계속 말한다(build_videos._evaluate_subcategory).
FALLBACK_MAX_PER_TAB = 20
