...
# 3회 이상 반복되는 **문자**를 2회로 축약한다.
#   ㅋㅋㅋㅋㅋ    -> ㅋㅋ
#   짜증나아아아  -> 짜증나아아   ("나"는 별개 음절이고 "아" 3개가 2개로 준다)
#   너무너무너무  -> 그대로       (문자 반복이 아니라 어절 반복이라 대상이 아니다)
#
# ⚠ 예전 주석은 "짜증나아아아 → 짜증나아"로 적혀 있었다. 실제 동작과 다르고,
#   그대로 믿으면 "축약 후에도 음절이 남는다"는 사실을 놓친다. 규칙(3회→2회)이
#   맞고 예시가 틀렸던 것이다. FYM에도 같은 예시가 남아 있다(HANDOFF 4.4).
