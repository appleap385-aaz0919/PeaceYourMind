...
THIRD_PARTY_PROGRAMS = (
    ...
    "위대한 유산",        # CTS 다큐
    # ★ 2026-08-28 추가 (사용자 신고 · 2.88). 둘 다 합집합 1,927건에서 1건씩 매치하고
    #   전부 제3범주다 — 오탐 0. 새 프로그램이 생기면 여기가 늘어야 한다.
    "땅끝에서 온 편지",    # CGN 선교사 다큐 시리즈
    "예수 영광의 면류관",   # CBS TV 프로그램 (CBSJOY가 클립을 올린다)
)
...
def drop_promotional(
    ...
    """공고·행사·홍보와 제3범주를 뺀다. (남은 것, 뺀 것+이유)를 돌려준다.

    ★ 2026-08-28 — **주제분에도 쓴다** (사용자 결정). 이름의 '폴백'은 이제
      정확하지 않지만 함수는 그대로 두고 호출부가 두 곳이 됐다.

      [왜 경계를 옮겼나 — 2.77 ④를 닫는다]
        전에는 폴백 전용이었다. 그래서 **주제어가 걸린 인터뷰·다큐가 주제분에
        그대로 남았다** — "세 자녀와 함께 심은 소망의 씨앗 | … | 땅끝에서 온
        편지 시즌3"이 `hope`로 태깅돼 5개 화면에 나갔다(사용자 신고).
        ★ 사용자 판단: **"주제어가 걸렸어도 인터뷰는 인터뷰다."**
        말씀 탭에서도 찬양 탭에서도 뺀다.

      실측(7일치 합집합 1,927건): 주제 태깅된 621건 중 **3건**이 빠진다
      (땅끝에서 온 편지 · 예수 영광의 면류관 · 원더풀우먼). 48/48 탭 20건 유지.
    ...
    ⚠ **뺀 자리를 되메우지 않는다.** 화면이 SUBCATEGORY_MIN_VIDEOS 미만이 되면
      ...
      **여기서 뺀 것을 다시 넣는 코드를 만들지 말 것.**
      주제분 쪽도 같다 — 빠진 만큼 그 탭의 폴백이 더 들어온다.
    """
...
def fill_balanced(
    ...
    [왜 균형을 맞추는가]
      ...
      unknown은 별도 몫을 두지 않는다. ⚠ 원래 이유는 "양쪽 토글에 노출되므로
      어느 쪽의 빈 화면도 만들지 않는다"였는데, 2026-08-28부터 unknown은 어느 탭에도
      안 나온다(visible_count 주석). 세분류 화면 쪽은 tab_pool이 unknown을 아예
      안 넘기므로 여기 오지 않고, 위기 풀 선정에서만 3차 순회로 들어올 수 있다.
...
def select_fallback_videos(
    ...
    *,
    ...
    [무엇을 담는가]
      target 형식만. ⚠ 2026-08-28까지는 "target + unknown"이었다 — unknown이
      양쪽 토글에 노출되던 시절의 규칙이다. 지금 unknown은 어느 탭에도 안 들어간다
      (visible_count 주석). 아래 pool 필터가 그 결정을 담고 있다.
...
def select_tab_layers(
    ...
    *,
    ...
    [탭 풀을 어떻게 나누나]
      앱의 visibleVideos()와 같은 기준이다 — **그 형식만** (2026-08-28부터).
      ⚠ 그 전에는 "그 형식 + unknown"이라 두 탭이 같은 영상을 가질 수 있었고,
        그래서 seen으로 한 번만 담고 부족분을 "이 탭에 보이는 수"로 셌다.
        unknown을 빼면서 두 탭은 겹치지 않게 됐지만 그 산술은 그대로 두었다 —
        지금은 항등이고, 되돌아가면 다시 뜻을 갖는다.
