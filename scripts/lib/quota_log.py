...
# GitHub Actions 배치가 하루에 쓰는 양 (scripts/lib/quota.py 예산표).
# 로컬은 Actions의 소모를 볼 수 없으므로, 이 양을 하루 예산에서 항상 빼둔다.
#
# 배치가 아직 안 돌았으면 "예약"(앞으로 쓸 몫), 이미 돌았으면 "계상"(이미 쓴 몫)이다.
# **이름만 다르고 금액은 같다** — 아래 [FYM 검산 기록] 참조.
#
# ★ 이 값은 **하루치 총액**이다. 배치 1회분이 아니다.
#   build.yml이 하루 2회(KST 18:30 · 06:30) 돌므로 1회분을 넣으면 절반만 비워둔다.
#
# [200 → 500 상향 — 하루 2회 전환 (2026-08-26)]
#   quota.py 산정기로 뽑은 1회 상한과 그 2배는 다음과 같다.
#
#     채널 15개(구)   64/회   2회  128
#     채널 22개(현재) 93/회   2회  186   ← 옛 예약 200은 여유가 14뿐이었다
#     채널 30개      126/회   2회  252   ← 옛 200으로는 **모자란다**
#     채널 50개      209/회   2회  418   ← PLAN.md 5.2절의 확장 목표치
#
#   500은 PLAN 목표인 50채널 2회(418)까지 덮는다. 채널을 늘릴 때마다 이 상수를
#   따라 고쳐야 하는 상황을 만들지 않으려고 목표치 기준으로 잡았다.
#   DAILY_CEILING 9,500 대비 5.3%라 로컬 예산에 실질적 제약이 없다
#   (채널 발굴 1회가 ~761이므로 남은 9,000으로 11회를 돌릴 수 있다).
#
#   ⚠ 옛 주석의 "약 150"은 채널 15개 시절 1회분 값이다. 실측 61·62·64·66이
#     그때의 산정치 64와 맞았고, 같은 산정기가 22채널에서 93을 낸다.
#
#   FYM에서 이 값이 문제가 됐던 이유는 7,900이 하루 예산의 79%였기 때문이다.
#   예약을 유지할지 풀지에 따라 로컬 도구의 가부가 갈렸고, 그 판단이 한 번
#   틀렸다가 되돌려졌다. PYM은 500이라 어느 쪽이든 실질적 차이가 없다 —
#   같은 회계 구조를 유지하되 그 논쟁이 재발할 여지가 사라진 것이다.
ACTIONS_BATCH_RESERVE = 500
