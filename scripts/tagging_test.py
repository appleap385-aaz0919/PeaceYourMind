...
MEDIA_CASES = [
    ...
    #
    # --- MM — 콘티 동점 분해는 sermon을 돌려주지 않는다 (2026-09-15 · HANDOFF 2.138) ---
    # ★ 배포본 말씀 탭 4곳에 나가던 203분 피아노 찬양 플레이리스트. '+'가 서술어를 이어
    #   "피아노 찬양"이 곡명으로 소비되고 나머지에 '묵상'만 남아 sermon/conti였다.
    #   ⚠ 근거가 channel(worship)이 아니라 conti(sermon)로 되돌아가면 규칙이 꺼진 것이다.
    (
        "[𝐏𝐥𝐚𝐲𝐥𝐢𝐬𝐭] 따뜻한 모닥불 + 피아노 찬양 | 묵상, 기도, 숙면, 집중, 공부 | Fireplace on Piano Worship",
        12216, "worship", WORSHIP, "channel",
    ),
    # ⛔ 나머지 구간이 sermon이라고 말하면 판정하지 않고 하류가 답한다 — 여기서는 채널
    ("말씀하소서 + 찬양합니다 | 오륜교회 저녁예배 설교", 1500, "sermon", SERMON, "channel"),
    # ⛔ 나머지 구간에 worship이 남으면 그대로 worship이다 (2.44의 원래 목적)
    ("주의 약속하신 말씀 위에서 + 오 신실하신 주 | 오륜교회 주일 4부예배 찬양", 900, "sermon", WORSHIP, "conti"),
    #
